...
public class UsuarioDAO {
    ...
    public void setNewUser(Usuario user) throws SQLException {
        String query = "insert into usuarios(tipo, nome, maezinha, sexo, nascimento, cpf, cnh, fone, logradouro, numero, complemento, bairro, senha) "
                     + "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, sha1(?))";
        
        PreparedStatement prep = conn.prepareStatement(query);
        
        prep.setString(1, user.getTipo() );
        prep.setString(2, user.getNome() );
        prep.setString(3, user.getMaezinha() );
        prep.setString(4, user.getSexo() );
        prep.setDate(5, user.getNascimento() );
        prep.setString(6, user.getCpf() );
        prep.setString(7, user.getCnh() );
        prep.setString(8, user.getFone() );
        prep.setString(9, user.getLogradouro() );
        prep.setInt(10, user.getNumero() );
        prep.setString(11, user.getComplemento() );
        prep.setString(12, user.getBairro() );
        prep.setString(13, user.getSenha() );
